...
for (j in 1:N) {
  for (i in 1:length(n)) {
    ...
    yrac <- sin(2 * pi * xrac) + 6*rnorm(n[i], mean = 0, sd = 0.1)  # True underlying function with Gaussian noise
    
    outlier_idx <- sample(1:n[i], size = ceiling(0.2*n[i]))  
    yrac[outlier_idx]<- yrac[outlier_idx] + rnorm(length(outlier_idx), mean = 5, sd = 1)  # Introduce outliers
