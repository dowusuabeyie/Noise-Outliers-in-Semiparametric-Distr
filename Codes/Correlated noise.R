...
sigma <- 0.1  # Standard deviation of errors

# Generate correlated errors using AR(1) process outside the loops
errors <- arima.sim(n = max(n), list(ar = phi), sd = sigma)
...
for (j in 1:N) {
  for (i in 1:length(n)) {
    ...
    # Extract relevant subset of errors for the current iteration
    errors_subset <- errors[1:n[i]]
    
    # Generate y values with correlated errors
    yrac <- sin(2 * pi * xrac) + 6 * errors_subset
